...
static void dumpNodesAndEdges(nlist_t nodelist, elist edgelist)
{
	FILE* fp = fopen("c:\\working\\gv\\debug-gv-nodes-and-edges.txt", "w+");
	if (fp != 0) {

		fprintf(fp, "nodes:%d\n", nodelist.size);
		for (int i = 0; i < nodelist.size; i++) {
			node_t* node = nodelist.list[i];
			textlabel_t* label = ND_label(node);
			if (label != 0) {
				fprintf(fp, "[%s]\n", label->text);
			}
			else {
				fprintf(fp, "[%d:%s]\n",ND_id(node), translate_type(ND_node_type(node)));
			}
		}
		fprintf(fp, "\n");
		fprintf(fp, "edges:%d\n", edgelist.size);
		for (int i = 0; i < edgelist.size; i++) 
		{
			...
			label = ND_label(tail);
			if (label != 0) {
				fprintf(fp, "(%s)", label->text);
			}
			else {
				fprintf(fp, "(%d:%s)", ND_id(tail), translate_type(ND_node_type(tail)));
			}
			...
			if (label != 0) {
				...
				fprintf(fp, "(%d:%s)", ND_id(head), translate_type(ND_node_type(head)));
			}
			fprintf(fp, "]\n");
		}
...
void init_rank(void)
{
	...
	while ((v = dequeue(Q)) != 0) {
		ND_rank(v) = 0;
		ctr++;
		for (i = 0; (e = ND_in(v).list[i]) != 0; i++)
			ND_rank(v) = MAX(ND_rank(v), ND_rank(agtail(e)) + ED_minlen(e));
		for (i = 0; (e = ND_out(v).list[i]) != 0; i++) {
...
static void dfs_enter_outedge(node_t * v)
{
	...
	for (i = 0; (e = ND_tree_in(v).list[i]) != 0 && (Slack > 0); i++)
...
static void dfs_enter_inedge(node_t * v)
{
	...
	for (i = 0; (e = ND_tree_out(v).list[i]) != 0 && (Slack > 0); i++)
...
static edge_t *enter_edge(edge_t * e)
{
	node_t *v;
	int outsearch;
	int lt = ND_lim(agtail(e));
	int lh = ND_lim(aghead(e));
	/* v is the down node */
	if (lt < lh ) {
		v = agtail(e);
		outsearch = FALSE;
...
static subtree_t *find_tight_subtree(Agnode_t *n)
{
	subtree_t       *rv;
	rv = NEW(subtree_t);
	rv->rep = n;
	rv->size = tight_subtree_search(n, rv);
	rv->par = rv;
	return rv;
}
...
void tree_adjust(Agnode_t *v, Agnode_t *from, int delta)
{
	...
	for (i = 0; (e = ND_tree_in(v).list[i]) != 0; i++) {
		w = agtail(e);
		if (w != from)
			tree_adjust(w, v, delta);
	}
	for (i = 0; (e = ND_tree_out(v).list[i]) != 0; i++) {
		w = aghead(e);
		if (w != from)
			tree_adjust(w, v, delta);
	}
...
int feasible_tree(void)
{
	Agnode_t *n;
	Agedge_t *ee;
	subtree_t **subTrees, *tree0, *tree1;
	...
	subTrees = N_NEW(N_nodes, subtree_t*);
	/* given init_rank, find all tight subtrees */
	for (n = GD_nlist(G); n; n = ND_next(n)) {
		if (ND_subtree(n) == 0) {
			subTrees[subtree_count] = find_tight_subtree(n);
			subtree_count++;
		}
	}

	/* incrementally merge subtrees */
	heap = STbuildheap(subTrees, subtree_count);
	while (STheapsize(heap) > 1) {
		tree0 = STextractmin(heap);
		if (0==(ee = inter_tree_edge(tree0))) {
			error = 1;
			break;
		}
	...
	free(heap);
	for (i = 0; i < subtree_count; i++) free(subTrees[i]);
	free(subTrees);
	if (error) return 1;
...
static int init_graph(graph_t * g)
{
	...
	Tree_edge.list = ALLOC(N_edges, Tree_edge.list, edge_t *);
	Tree_edge.size = 0;
...
int rank2(graph_t * g, int balance, int maxiter, int search_size)
{
	...
	//dumpNodesAndEdges(Tree_node, Tree_edge);
	//int cnt = 0;
	//char display_e[256] = { 0 };
	//char display_f[256] = { 0 };

	while ((e = leave_edge()) != 0) {
		...
		f = enter_edge(e);
		//printEdge(f, display_f, sizeof(display_f));
