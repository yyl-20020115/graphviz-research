...
//	int nn =0, ne=0;
	...
	attrsym_t* N_level;
#endif
	//export_graph(g);//46 nodes
	edgelabel_ranks(g);//no edge label rank
	...
	class1(g);//build virtual edges for the graph (clone virtual)
...
//	int s = GD_comp(g).size;//s==1
	...
	{
		//no min max set, decompose not called
		decompose(g, 0);
	}
#ifdef ALLOW_LEVELS
...
extern int rank2(graph_t *, int, int, int);
