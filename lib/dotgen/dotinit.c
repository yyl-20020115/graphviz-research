	...
	if (fp != 0) {
		for (i = GD_minrank(g); i <= GD_maxrank(g); i++) {
			...
			for (j = 0; j < rank[i].n; j++) {
				u = rank[i].v[j];
				rcnt++;
				fprintf(fp, "%s\n", ND_label(u)!=0 && ND_label(u)->text!=0 ? ND_label(u)->text : "<virtual>");
				//if (streq(agnameof(u), "virtual"))
				//	fprintf(fp, " %x", u);
				//else
				//	fprintf(fp, " %s", agnameof(u));

			}
			fprintf(fp, "\n");
		}
		fprintf(fp, "rank count = %d\n", rcnt);
		fclose(fp);
	}
}
...
static void dotLayout(Agraph_t * g)
{
	...
	//-1
	//GD_flags(g) |= ET_SPLINE
	setEdgeType(g, ET_SPLINE); //graph's attribute line type = Spline
	//no aspect, set next-iter = 0 and badgraph = 0,asp = 0
	asp = setAspect(g, &aspect);
	//no subgraph
	dot_init_subg(g, g); //all subgraphs having same root
	...
	dot_sameports(g);//not called
	dot_splines(g);
	if (mapbool(agget(g, "compound"))) {
		dot_compoundEdges(g);//not called
	}
}
...
static void doDot(Agraph_t* g)
{
	...
	int i;
	pack_info pinfo;
	int Pack = getPack(g, -1, CL_OFFSET); //=-1
...
{
	//if there is any node, we do dot algorithm
	if (agnnodes(g))
	{
		doDot(g);
	}
