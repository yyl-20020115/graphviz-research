...
static edge_t *bot_bound(edge_t *, int);
static boolean pathscross(node_t *, node_t *, edge_t *, edge_t *);
static graph_t *cl_bound(graph_t*, node_t *, node_t *);
static int cl_vninside(graph_t *, node_t *);
static void completeregularpath(path *, edge_t *, edge_t *,
	pathend_t *, pathend_t *, boxf *, int, int);
static int edgecmp(edge_t **, edge_t **);
static void make_flat_edge(graph_t*, spline_info_t*, path *, edge_t **, int, int, int);
static void make_regular_edge(graph_t* g, spline_info_t*, path *, edge_t **, int, int, int);
static boxf makeregularend(boxf, int, double);
static boxf maximal_bbox(graph_t* g, spline_info_t*, node_t *, edge_t *, edge_t *);
static Agnode_t *neighbor(graph_t*, node_t *, edge_t *, edge_t *, int);
static void place_vnlabel(node_t *);
static boxf rank_box(spline_info_t* sp, graph_t *, int);
static void recover_slack(edge_t *, path *);
static void resize_vn(node_t *, int, int, int);
static void setflags(edge_t *, int, int, int);
static int straight_len(node_t *);
static Agedge_t *straight_path(edge_t *, int, pointf *, int *);
static Agedge_t *top_bound(edge_t *, int);
...
//should merge? node is virtual and (out size>1 or in size>1)
...
static void swap_bezier(bezier * old, bezier * new)
{
	...
	new->sflag = old->eflag;
	new->eflag = old->sflag;
	new->sp = old->ep; //start point = end point
	new->ep = old->sp; //end point = start point
}

/* swap_spline:
 * spline is a list of beziers
 */
